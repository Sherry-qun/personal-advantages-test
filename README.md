...
- `.gitignore`：排除本地配置、日志、密钥和临时文件
- `LICENSE`：项目的专有使用许可与权利声明
...
本项目采用保留所有权利的专有许可证，而不是允许他人自由复制和商用的开源许可证。公开 GitHub 仓库只代表代码可被查看，不代表他人获得复制、修改、转售或再授权的权利，具体以 `LICENSE` 为准。

正式上线前，建议将 `LICENSE` 第一行的权利人名称改为你的真实姓名、公司名称或依法登记的经营主体名称。
